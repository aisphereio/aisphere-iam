...
最新 Envoy Gateway + Casdoor OIDC + IAM ExtAuth 接入说明见 [`docs/envoy-casdoor-oidc-extauth.md`](docs/envoy-casdoor-oidc-extauth.md)。
...
Casdoor OIDC Provider
  ...
  -> IAM ExtAuth maps external identity to Aisphere principal
  -> upstream services read trusted x-aisphere-* headers
```
...
| 能力 | 说明 |
|---|---|
| `/v1/extauth/check` | Envoy Gateway External Authorization 标准检查入口 |
| `x-aisphere-principal` | IAM 返回给 Gateway 注入上游的内部主体 |
| `x-aisphere-internal-jwt` | IAM 签发的短期内部 JWT，用于后端二次校验 |
| `external_sub -> user` | Casdoor subject 到 Aisphere user 的映射 |
