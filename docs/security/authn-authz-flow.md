...
- Gateway removes inbound trusted identity headers before injecting its own verified principal.
...
```text
GET /v1/users
  -> check iam:organization#read

POST /v1/users
  -> check iam:organization#manage

DELETE /v1/users/{username}
  -> check iam:organization#manage
```

The `legacy_resource=iam:local_user` metadata is recorded with the audit event so logs still identify the concrete target. Long term, this legacy API should be migrated into a generated proto identity-admin service with its own `iam:local_user` resource.
...
- Local user legacy APIs are explicitly protected by handler-level `accessx` checks.
