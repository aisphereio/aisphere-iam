...
.PHONY: help init tools tools-local check-tools api deploy proto-check config wire generate build docker run test tidy verify clean

help:
	@echo "Aisphere IAM targets:"
	@echo "  make init         install local toolchain into .bin"
	@echo "  make tools        install released codegen tools into .bin"
	...
	@echo "  make wire         generate dependency injection code if configured"
